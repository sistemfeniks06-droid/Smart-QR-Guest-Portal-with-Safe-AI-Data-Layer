...
Razumem šta se dešava! Sve jeste u jednom dokumentu i u jednom bloku, ali
problem je u tome kako kopiraš tekst iz Google AI Studija (koji koristiš, kako
vidim na slici).

Kada mišem prevlačiš preko teksta u AI Studiju, on kopira "šminku" (lep izgled
teksta), a izgubi se sirovi kod (tarabe #, zvezdice *) koji je GitHubu potreban
da bi to lepo prikazao.

Zato ti se prošli put sve slepilo i bilo je crveno!

Kako da iskopiraš tačno i iz jednog poteza:

1.  Skroluj malo niže do ovog velikog, sivog/crnog prozora ispod gde počinje
    kod.
2.  NEMOJ prevlačiti mišem preko teksta.
3.  Samo pređi mišem preko tog sivog prozora i u gornjem desnom uglu tog prozora
    će se pojaviti mala ikonica "Copy" (izgleda kao dva mala kvadrata/papira
    koja se preklapaju).
4.  Klikni na tu ikonicu! (Pisaće ti "Copied").
5.  Zatim idi na GitHub i samo uradi Paste (Nalepi).

Sada će apsolutno sve (od naslova, preko Tech Stack-a, pa do samog kraja) biti
iskopirano kako treba iz ovog jednog bloka ispod:
...
🛠️ Tech Stack
  ...
  - Backend & Database: Firebase Firestore (NoSQL for dynamic tenant routing),
    Firebase Hosting.
  - AI Infrastructure: JSON-LD / Schema.org structured metadata layer with
    security constraints.
  - Design Framework: iOS Dynamic Island UI, Glassmorphism backdrop filters, and
...
This project was developed following the Orient → Plan → Build → Transfer (OPBT)
framework:

1.  Orient: Identified friction points in hospitality QR code maintenance and AI
    agent access safety.
2.  Plan: Architecture mapped for single-page iOS glassmorphism UI paired with
    Firestore NoSQL client lookup and structured JSON-LD AI limits.
3.  Build: Implemented full dynamic routing, feedback branching, and accessible
    ARIA labels.
4.  Transfer: Deployed live on Firebase Hosting and open-sourced on GitHub with
